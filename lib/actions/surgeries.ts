...
const SurgeryCaseSchema = z.object({
  ...
  endOfAnesthesiaTime: z.string().optional(),
...
export async function createSurgeryCase(clinicId: string, dayId: string, formData: FormData) {
  const raw = {
    ...
    inductionTime: formData.get("inductionTime") as string || undefined,
    ...
    endOfAnesthesiaTime: formData.get("endOfAnesthesiaTime") as string || undefined,
  ...
  await prisma.surgeryCase.create({
    data: {
      ...
      inductionTime: parseTime(dayDate, parsed.data.inductionTime),
      ...
      endOfAnesthesiaTime: parseTime(dayDate, parsed.data.endOfAnesthesiaTime),
